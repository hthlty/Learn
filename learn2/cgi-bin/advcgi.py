...
class AdvCGI(object):
    ...
    Advandced CGI Demo</title></head>
    <body><h2>Advanced CGI Demo Form</h2>
    <form method=post actio="%s" enctype="multipart/form-data">
    ...
    (<i>at least on required</i>)</h3>
    %s
    <h3>Enter file to uupload <small>(max size 4k)</samll></h3>
    <input type=file name=upfile value="%s" size=45>
    <p><input type=submmit>
    </form></body></html>'''

    langSet = ('Pytho','Ruby','Java','C++','PHP','C','JavaScript')
    langItem = '<input type=checkbox name=lang value="%s"%s>%s\n'

    def getCPPCookies(self):  #read cookie from client
    ...
    def showForm(self):
        ...
        for eachLang in AdvCGI.langSet:
            langStr.append(AdvCGI.langItem %(eachLang,
                    'CHECKED' if eachLang in self.langs else '',eachLang))
        ...
        if not ('user' in self.cookies and self.cookies['user']):
            ...
            userCookie = cookStatus = self.cookies['user']

        print('%s%s' %(AdvCGI.header,AdvCGI.formhtml
                       %(AdvCGI.url,cookStatus,userCookie,self.who,
                         ''.join(langStr),self.fn)))
    errhtml = '''<html><head><title>
    Advance CGI Demo</title></head>
    <body><h3>ERROR</h3>
    <b>%s </b><p>
    ...
    def doResult(self):#display result page
        ...
        if len(filedata) == MAXBYTES and f.read():
            filedata = '%s%s'%(filedata,
                        '...<b><i>(file truncate due to size)</i></b>')
        ...
        # see if user cookie set up yet
        if not ('user' in self.cookies and self.cookies['user']):
            cookStatus = '<i>(cookie hass not been set yet)</i>'
            userCook = ''
        else:
    ...
    def go(self): #determine which page to return
        ...
        if 'lang' in form:
            ...
            self.error = 'At least one language requird.'
